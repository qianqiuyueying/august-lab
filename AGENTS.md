...
本文件为 Codex (Codex.ai/code) 提供在本代码库中工作的指引。
...
写完代码后先运行 `/simplify` 技能，让 Codex 检查代码质量、复用性和潜在问题。
...
**后端单元测试和集成测试、前端组件测试全部通过之后**，Codex 应主动发起 e2e 测试流程：

1. **设计测试路径**：根据本次改动的范围，Codex 自动分析需要覆盖的用户操作路径，并向用户提出 e2e 测试方案
