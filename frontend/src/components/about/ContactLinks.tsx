...
const icons: Record<string, FC<{ className?: string }>> = {
  ...
  twitter: ({ className }) => (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" width="20" height="20">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161.759l1.603 2.113H6.88l8.203 10.814 2.012-2.644L8.892 5.009h8.191z" />
    </svg>
  ),
  email: ({ className }) => (
  ...
  return (
    <section className="flex flex-wrap items-center gap-5">
      {contacts.map((contact, index) => {
        const Icon = icons[contact.platform] ?? defaultIcon;
        const label = contact.platform === 'custom' ? (contact.name || '链接') : contact.platform.charAt(0).toUpperCase() + contact.platform.slice(1);
        return (
          <a
            key={index}
